...
let blockHeight = 10;
let blockColumns = 8;
let blockRows = 4; //number of rows to hit
let blockMaxRows = 10; //setting total limit for rows
let blockCount = 0;
...
function movePlayerWithMouse(e) {
  ...
  // Get the mouse x position relative to the board
  let mouseX = e.clientX - board.getBoundingClientRect().left;

  // Ensure the player stays within the board boundaries
  let nextPlayerX = mouseX - player.width / 2;
  nextPlayerX = Math.max(0, Math.min(boardWidth - player.width, nextPlayerX));
...
function update() {
  ...
  //bouncing of the ball off player paddle
  if (topCollision(ball, player) || bottomCollision(ball, player)) {
    ball.velocityY *= -1; //  up or down of the ball
  } else if (leftCollision(ball, player) || rightCollision(ball, player)) {
    ball.velocityX *= -1; // left or right movement
  }

  if (ball.y <= 0) {
    // if ball touches top of canvas reverse the direction of the ball
    ball.velocityY *= -1;
  } else if (ball.x <= 0 || ball.x + ball.width >= boardWidth) {
    // similarly if ball touches left or right of canvas reverse the direction
    ball.velocityX *= -1;
  } else if (ball.y + ball.height >= boardHeight) {
    // if ball touches bottom of canvas GAME OVER
    context.font = "22px sans-serif";
    context.fillText("Game Over, Press 'SPACE' to Restart", 70, 300);
    gameOver = true;
  }
  ...
  // If all block are cleared moving to the
  //next level
  if (blockCount == 0) {
  ...
  context.fillText(`Total Score: ${score}`, 10, 25);
}
